      ...
      (set, get) => ({
        ...
        checkUser: async (): Promise<boolean> => {
          try {
            ...
            const response = await axiosInstance.get("/employees/CheckUser", {
              headers: {
...
export const checkEmployeeAuthStatus = async (): Promise<boolean> => {
  const { checkUser } = useEmployeeAuthStore.getState();
