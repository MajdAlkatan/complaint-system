...
class AuthRepository extends GetxService {
  ...
  Future<ApiResponse<LoginResponse>> login(
    ...
    if (response.success && response.data != null) {
      ...
      // Fetch user data after login - FIXED: Use the user data from login response
      // The login response already contains user data in the API response
      // Check if response.data!.user exists (from LoginResponse)

      // First, try to get user from the login response
      if (response.data!.user != null) {
        // Store user data directly from login response
        final userMap = {
          'citizen_id': response.data!.user!.citizenId,
          'full_name': response.data!.user!.fullName,
          'email': response.data!.user!.email,
          'phone': response.data!.user!.phone,
          'birth_of_date': response.data!.user!.birthOfDate,
          'created_at': response.data!.user!.createdAt,
          'updated_at': response.data!.user!.updatedAt,
        };
        _storageService.write('user', userMap);
      } else {
        // If not in login response, fetch from profile endpoint
    ...
    print("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM , ${response}"  );
  ...
  User? get currentUser {
    final userData = _storageService.read<Map<dynamic, dynamic>>('user');
    if (userData != null) {
